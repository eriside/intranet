...
@section("content")
    @if($roles->contains('Inaktive Mitarbeiter'))
        <div class="container my-5">
            <h1 class="text-center text-white mb-4">Inaktive Mitarbeiter</h1>
        ...
        <div class="row mt-4 d-flex justify-content-center align-items-stretch">
            @foreach($mitarbeiter as $mitarbeiter)
                @foreach($dienstgrade as $dienstgrad)
                    @if($dienstgrad->id == $mitarbeiter->dienstgrad and $mitarbeiter->aktiv)
                        <div class="col-11 col-md-4 col-lg-3 mx-2 mb-4" data-title="{{$mitarbeiter->name}} {{$mitarbeiter->dienstnummer}} {{$dienstgrad->name}}">
                            <div class="card h-100 shadow" style="background-color: #2e3338; border: none; border-radius: 15px;">
                                <div class="card-body text-white">
                                    ...
                                    <p class="mb-1">Dienstnummer: <strong>{{$mitarbeiter->dienstnummer}}</strong></p>
                                    <p class="mb-3">Dienstgrad: <strong>{{$dienstgrad->name}}</strong></p>
                                    <div class="d-flex justify-content-between">
                                        <a href="{{url('/intranet/verwaltung/mitarbeiter/view', $mitarbeiter->id)}}" class="btn btn-primary btn-sm rounded-pill">
                                            Ansehen
                                        </a>
                                        <a href="{{url('/intranet/verwaltung/mitarbeiter/edit', $mitarbeiter->id)}}" class="btn btn-danger btn-sm rounded-pill">
                                            Bearbeiten
                                        </a>
                                    </div>
                                </div>
